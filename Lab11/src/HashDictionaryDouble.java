 ...
 * 
 * based on HashDictionaryLinear source code - from @author Greg
 * @author Sean Yan
 * 
 */
public class HashDictionaryDouble<K, V> implements DictionaryInterface<K, V> 
{
	...
	{
		// for double hashing, we hash again using a smaller prime number
	...
	private V doubleHash(K key, V value) 
	{
		...
		// if the key is not duplicated, do the double hashing
		else
		{
			...
			int pobe_hash = probeValue(key);
			// use double hashing
			while (dictionary[hash] != null) 
			{
				hash = (hash + pobe_hash) % currentCapacity;
			}
			numberOfElements++;
